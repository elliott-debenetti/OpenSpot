...
class TemplateBasedLabeler:
    ...
    def __init__(self, image_dir, template_file, output_file='annotations.json'):
        ...
        self.output_file = self.image_dir / output_file
    ...
    def draw(self):
        ...
        dst_x_start = int(max(0, x_offset))
        dst_y_start = int(max(0, y_offset))
        dst_x_end = int(min(canvas_width, x_offset + zoomed_width))
        dst_y_end = int(min(canvas_height, y_offset + zoomed_height))
        
        # Source region in zoomed image
        src_x_start = int(max(0, -x_offset))
        src_y_start = int(max(0, -y_offset))
        src_x_end = int(src_x_start + (dst_x_end - dst_x_start))
        src_y_end = int(src_y_start + (dst_y_end - dst_y_start))
    ...
    def draw_instructions(self):
        ...
        instructions = [
            ...
            f"Spots: {len(self.current_occupancy)} | Occupied: {occupied_count} | Empty: {empty_count} | Zoom: {self.zoom_level:.1f}x",
            "",
            "FAST MODE: Click spot → 1 (occupied) or 2 (empty) | O: All occupied | E: All empty | C: Copy previous",
            "Zoom: +/- | Drag to Pan | L: Toggle text",
    ...
    def draw_help_overlay(self):
        ...
        help_text = [
            ...
            "  • Drag anywhere to pan around the image",
            "",
    ...
    def mouse_callback(self, event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            # Check if clicking on ROI first
            ...
            # If not clicking on ROI, start panning
            if not clicked_roi:
            ...
            if self.is_panning and self.pan_start is not None:
                dx = abs(x - self.pan_start[0])
                dy = abs(y - self.pan_start[1])
                # If movement is small (< 5 pixels), treat as click to deselect
                if dx < 5 and dy < 5:
                    self.selected_roi = None
                    self.draw()
    ...
    def zoom_in(self, factor=1.2):
        """Zoom in by factor, centered on viewport center."""
        ...
        if new_zoom <= self.max_zoom:
            ...
            # Adjust pan offset to keep same center point
            self.pan_offset[0] = center_x - img_x * self.zoom_level
            self.pan_offset[1] = center_y - img_y * self.zoom_level
            
            self.draw()
    
    def zoom_out(self, factor=1.2):
        """Zoom out by factor, centered on viewport center."""
        new_zoom = self.zoom_level / factor
...
def main():
    ...
    try:
        labeler = TemplateBasedLabeler(args.image_dir, args.template)
        labeler.run()
    except Exception as e:
        print(f"Error: {e}")
        import traceback
